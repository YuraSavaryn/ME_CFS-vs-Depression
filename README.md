...
Веб-додаток розроблений для встановлення діагнозу в пацієнта на основі даних, чи у пацієнта депресія, чи синдром
...
2. Встановіть усі необхідні бібліотеки з файлу requirements.txt у своє віртуальне середовище venv

3. Підключіть PostgreSQL до проекту через файл .env

4. Запустіть main.py з папки app

5. Сервер запуститься на http://127.0.0.1:8000

### Як викликати /train/model ?

Після запуску сервера перейдіть на http://127.0.0.1:8000/docs

Потрапивши на swagger документцію, у секції /train ви можете викликати /model
...
1. data (зберігає необроблені дані датасету стану пацієнтів та їх діагнозів)


2. clean_data (зберігає оброблені дані датасету стану пацієнтів та їх діагнозів)


3. predictions (зберігає усі передбачення, які робить модель і під час тренування і під час) 


4. inference_inputs (зберігає нові дані, які поступають на /inference/predict від пацієнтів)
